export async function sendOtpEmail({ to, name, code }) {
  ...
  if (!apiKey) {
    console.info(`[auth-otp] Demo OTP for ${to}: ${code}`);
  ...
  try {
    ...
    if (!res.ok) return { delivered: false, demo: true };
  ...
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        html: `<p>${label}</p><p>${input.name} · ${input.email} · ${input.phone || "—"} · ${input.ip || "—"} · ${input.createdAt}</p>`,
      }),
    });
